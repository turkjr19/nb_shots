...
nhl_rink_plot <- function () {
  ...
  ggplot() +
    ...
    # homeplate
    geom_polygon(aes(x = c(-89, -69, -54, -54, -69, -89, -89),
                     y = c(3, 22, 22, -22, -22, -3, 3)),
                 fill = NB_gold, alpha = .25) + # away end
    geom_polygon(aes(x = c(89, 69, 54, 54, 69, 89, 89),
                     y = c(3, 22, 22, -22, -22, -3, 3)),
                 fill = NB_gold, alpha = .25) + # home end
...
NB_gold <- "#f5d20e"
# plot shots
rink + geom_point(data = goals, aes(x=x.plot, y=y.plot)) +
  ggtitle("goals") +
